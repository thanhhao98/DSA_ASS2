...
/************************************************************************
 * This section is for Global function of linklist and AVL
 ************************************************************************/
template <class T>
void makeList(AVLNode<T> *pR, L1List<T> &list){
    if(!pR) return;
    makeList(pR->_pLeft,list);
    list.push_back(pR->data);
    makeList(pR->_pRight,list);
    return;
}
